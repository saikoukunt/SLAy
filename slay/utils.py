...
def calc_mean_and_std_wf(
    ...
    if os.path.exists(mean_wf_path) and (not return_std or os.path.exists(std_wf_path)):
        ...
        mean_wf *= get_bits_to_uV_factor(params)  # convert from bits to uV
...
def get_bits_to_uV_factor(params):
    meta_path = params["data_path"].replace(".bin", ".meta")
    meta = slay.SGLXMetaToCoords.readMeta(Path(meta_path))
